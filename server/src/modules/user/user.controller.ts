...
export class UserController {
  ...
  async updateFreelancerProfile(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (user?.role !== 'FREELANCER') { res.status(403).json({ success: false, message: 'Access denied' }); return }
  ...
  async updateCompanyProfile(req: Request, res: Response): Promise<void> {
    try {
      const user = await prisma.user.findUnique({ where: { id: req.user!.userId }, select: { role: true } })
      if (user?.role !== 'COMPANY') { res.status(403).json({ success: false, message: 'Access denied' }); return }
