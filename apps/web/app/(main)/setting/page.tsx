...
function ChevronRight() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4 text-[#9ca3af]">
...
export default function SettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9]">
      <MainHeader />
      <div className="mx-auto max-w-2xl px-4 pb-28 pt-20">
        ...
        {page !== 'menu' && (
          <div className="mb-4 flex items-center gap-3">
            ...
            <h1 className="text-lg font-extrabold text-[#1b1c1a]">{pageTitles[page]}</h1>
          </div>
        )}
                ...
                <div className="overflow-hidden rounded-2xl border border-[#e2e8f0] bg-white shadow-sm">
                  {section.items.map((item, i) => (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 px-4 py-3.5 text-left transition hover:bg-[#f8fafc] active:scale-[0.99] ${
                        i > 0 ? 'border-t border-[#f1f5f9]' : ''
                      }`}
                    >
                      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#f1f5f9] text-lg">
                        {item.icon}
                      </span>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-bold text-[#1b1c1a]">{item.label}</p>
