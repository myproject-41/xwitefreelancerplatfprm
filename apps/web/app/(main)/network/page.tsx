...
function Avatar({ name, image, size = 'md' }: { name: string; image: string | null; size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl' }) {
  ...
  return (
    <div className={`${sz} rounded-full overflow-hidden bg-[#BBDEFB] flex items-center justify-center shrink-0 border-2 border-white shadow-sm font-bold text-[#0160B9]`}>
      {image ? (
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. React Developers India" className="w-full rounded-lg border border-[#bfc7d1] px-3 py-2.5 text-sm text-[#1b1c1a] outline-none focus:border-[#0160B9] focus:ring-1 focus:ring-[#0160B9]/30" />
          </div>
          <div>
            <label className="text-xs font-bold text-[#404850] block mb-1">Description</label>
            <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={3} placeholder="What is this group about?" className="w-full rounded-lg border border-[#bfc7d1] px-3 py-2.5 text-sm text-[#1b1c1a] outline-none focus:border-[#0160B9] focus:ring-1 focus:ring-[#0160B9]/30 resize-none" />
          </div>
          <p className="bg-[#BBDEFB]/30 rounded-lg p-3 text-xs text-[#0160B9]">Your connections and followers will be able to join this group.</p>
        </div>
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. AI Design Summit 2026" className="w-full rounded-lg border border-[#bfc7d1] px-3 py-2.5 text-sm text-[#1b1c1a] outline-none focus:border-[#0160B9] focus:ring-1 focus:ring-[#0160B9]/30" />
          </div>
          <div>
            <label className="text-xs font-bold text-[#404850] block mb-1">Date & Time *</label>
            <input type="datetime-local" value={date} onChange={(e) => setDate(e.target.value)} className="w-full rounded-lg border border-[#bfc7d1] px-3 py-2.5 text-sm text-[#1b1c1a] outline-none focus:border-[#0160B9] focus:ring-1 focus:ring-[#0160B9]/30" />
          </div>
          <div>
            <label className="text-xs font-bold text-[#404850] block mb-2">Event Type</label>
            <div className="flex gap-2">
              {(['virtual', 'physical'] as const).map((t) => (
                <button key={t} onClick={() => setType(t)} className={`flex-1 py-2 rounded-full text-sm font-bold border-2 transition ${type === t ? 'border-[#0160B9] bg-[#0160B9]/5 text-[#0160B9]' : 'border-[#bfc7d1] text-[#404850]'}`}>{t === 'virtual' ? 'Virtual' : 'In-Person'}</button>
              ))}
            </div>
          </div>
          {type === 'physical' && (
            <div>
              ...
              <input type="text" value={loc} onChange={(e) => setLoc(e.target.value)} placeholder="City, Venue" className="w-full rounded-lg border border-[#bfc7d1] px-3 py-2.5 text-sm text-[#1b1c1a] outline-none focus:border-[#0160B9] focus:ring-1 focus:ring-[#0160B9]/30" />
            </div>
          )}
        </div>
        <div className="px-5 pb-5 flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-full border border-[#bfc7d1] text-[#404850] text-sm font-bold hover:bg-[#efeeeb] transition">Cancel</button>
          <button onClick={() => { if (!name.trim()) return toast.error('Name required'); if (!date) return toast.error('Date required'); setLoading(true); setTimeout(() => { toast.success(`Event "${name}" created!`); setLoading(false); onClose() }, 800) }} disabled={loading} className="flex-1 py-2.5 rounded-full bg-[#0160B9] text-white text-sm font-bold disabled:opacity-60 transition">{loading ? 'Creating…' : 'Create Event'}</button>
        </div>
      </div>
    </div>
  )
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. The Design Weekly" className="w-full rounded-lg border border-[#bfc7d1] px-3 py-2.5 text-sm text-[#1b1c1a] outline-none focus:border-[#0160B9] focus:ring-1 focus:ring-[#0160B9]/30" />
          </div>
          <div>
            <label className="text-xs font-bold text-[#404850] block mb-1">Description *</label>
            <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={3} placeholder="What topics will you cover?" className="w-full rounded-lg border border-[#bfc7d1] px-3 py-2.5 text-sm text-[#1b1c1a] outline-none focus:border-[#0160B9] focus:ring-1 focus:ring-[#0160B9]/30 resize-none" />
          </div>
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[{ value: 'daily', label: 'Daily' }, { value: 'weekly', label: 'Weekly' }, { value: 'biweekly', label: 'Bi-weekly' }, { value: 'monthly', label: 'Monthly' }].map((f) => (
                <button key={f.value} onClick={() => setFreq(f.value)} className={`py-2 rounded-lg text-sm font-bold border-2 transition ${freq === f.value ? 'border-[#0160B9] bg-[#0160B9]/5 text-[#0160B9]' : 'border-[#bfc7d1] text-[#404850] hover:border-[#0160B9]'}`}>{f.label}</button>
              ))}
            </div>
          </div>
        </div>
...
function PendingInvitations({ pending, onAccept, onIgnore }: { pending: any[]; onAccept: (id: string) => void; onIgnore: (id: string) => void }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#e3e2df] shadow-sm overflow-hidden mb-6">
        ...
        {pending.slice(0, 3).map((req: any) => {
          ...
          return (
            <div key={req.id} className="p-4 hover:bg-[#faf9f6] transition-colors">
              ...
              <div className="flex gap-2">
                <button onClick={() => onIgnore(req.id)} className="flex-1 py-1.5 rounded-full border border-[#0160B9] text-[#0160B9] text-xs font-bold hover:bg-[#0160B9]/5 transition">Ignore</button>
                <button onClick={() => onAccept(req.id)} className="flex-1 py-1.5 rounded-full bg-[#0160B9] text-white text-xs font-bold shadow-sm transition active:scale-95">Accept</button>
              </div>
            </div>
          )
        })}
      ...
      {pending.length > 3 && (
        <button className="w-full py-3 text-sm font-bold text-[#0160B9] hover:bg-[#faf9f6] transition border-t border-[#efeeeb]">
    ...
    <div className="space-y-10">
      ...
      <section>
        <div className="flex items-end justify-between mb-6">
          ...
          <p className="text-xs font-semibold text-[#707881]">{allCompanySuggestions.length} companies</p>
        </div>

        {allCompanySuggestions.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {allCompanySuggestions.map((co: any, i: number) => {
                  ...
                  <div className={`h-16 bg-gradient-to-br ${gradients[i % gradients.length]} relative`}>
                    <div className="absolute -bottom-6 left-6 w-14 h-14 bg-white rounded-lg shadow-sm border border-[#e3e2df] flex items-center justify-center overflow-hidden">
                      {image ? (
                        ...
                        <span className="text-xl font-black text-[#0160B9]">{name[0]}</span>
                      )}
                    </div>
                  </div>
                  <div className="p-6 pt-9">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <h3 className="font-[Manrope] font-bold text-lg group-hover:text-[#0160B9] transition-colors leading-tight">{name}</h3>
                      {isVerified && <VerifiedBadge size="sm" />}
                    </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-center gap-2 py-2 rounded-full border-2 font-bold text-sm transition active:scale-95 ${isFollowed ? 'border-[#e3e2df] bg-[#e3e2df] text-[#404850]' : 'border-[#0160B9] text-[#0160B9] hover:bg-[#0160B9]/5'}`}
                    >
      ...
      <section>
        ...
        {allPeopleSuggestions.length > peopleLimit && (
          <div className="flex justify-center mt-4">
            <button
              onClick={() => setPeopleLimit(v => v + 8)}
              className="px-6 py-2 rounded-full border-2 border-[#0160B9] text-[#0160B9] font-bold text-sm hover:bg-[#0160B9]/5 transition active:scale-95"
            >
      ...
      <section className="bg-[#0160B9] rounded-xl p-6 text-white shadow-lg">
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={() => router.push('/network?section=connections')} className="px-4 py-2 rounded-full bg-white text-[#0160B9] font-bold text-sm hover:bg-white/90 transition active:scale-95">View Connections</button>
...
function ConnectionsSection({ pending, connections, onAccept, onIgnore, onRemove }: any) {
  ...
  return (
    <div>
      ...
      <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search your connections…" className="w-full rounded-full bg-[#efeeeb] border-none px-4 py-2.5 text-sm text-[#1b1c1a] outline-none focus:ring-2 focus:ring-[#0160B9]/30 mb-5" />
            ...
            return (
              <div key={c.connectionId} className="bg-white rounded-xl border border-[#e3e2df] shadow-sm p-4 flex items-center gap-4 hover:bg-[#faf9f6] transition-colors">
                ...
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => handleOpenChat(c.user?.id)} className="flex items-center gap-1 px-2 py-1 rounded-full border border-[#0160B9] text-[#0160B9] text-xs font-bold hover:bg-[#0160B9]/5 transition"><Icons.Message /> Msg</button>
              ...
              return (
                <div key={f.follower.id} className="bg-white rounded-xl border border-[#e3e2df] shadow-sm p-4 flex items-center gap-3 hover:bg-[#faf9f6] transition-colors">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full border text-xs font-bold transition shrink-0 ${isConnected ? 'border-[#bfc7d1] text-[#707881]' : 'border-[#0160B9] text-[#0160B9] hover:bg-[#0160B9]/5'}`}
                  >
        ...
        {MOCK_GROUPS.map((g, i) => (
          <div key={g.id} className="bg-white rounded-xl border border-[#e3e2df] shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            ...
            <div className="p-5">
              ...
              <button className="w-full py-2 rounded-full border border-[#0160B9] text-[#0160B9] text-sm font-bold hover:bg-[#0160B9]/5 transition active:scale-95">View Group</button>
            </div>
          </div>
        ))}
      </div>
            ...
            <div key={n.id} className="bg-white rounded-xl border border-[#e3e2df] shadow-sm p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-[#0160B9] font-medium">by {n.author}</p>
        ...
        <div className="max-w-screen-xl mx-auto px-4 md:px-8 pt-20 md:pt-24">
          <div className="rounded-2xl bg-[#0160B9] text-white p-8 flex flex-col sm:flex-row items-center gap-6 shadow-lg mb-6">
            ...
            <div className="flex gap-3 shrink-0">
              <Link href="/login" className="px-5 py-2.5 rounded-full bg-white text-[#0160B9] text-sm font-bold hover:bg-[#f0f7ff] transition active:scale-95">
                Sign in
              </Link>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 mb-5 lg:hidden scrollbar-hide">
          {navItems.map((item) => (
            <button
              ...
              className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-bold transition-all ${activeSection === item.id ? 'bg-[#0160B9] text-white shadow-sm' : 'bg-white border border-[#e3e2df] text-[#404850] hover:border-[#0160B9] hover:text-[#0160B9]'}`}
            >
              {item.label}
              ...
              <nav className="space-y-1">
                {navItems.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => navigate(item.id)}
                    className={`flex items-center justify-between w-full p-3 rounded-lg transition-colors text-left group ${activeSection === item.id ? 'bg-[#BBDEFB]/30 text-[#0160B9]' : 'hover:bg-[#f4f3f0] text-[#404850]'}`}
                  >
                    <div className="flex items-center gap-3">
                      <span className={`transition-colors ${activeSection === item.id ? 'text-[#0160B9]' : 'text-[#707881] group-hover:text-[#0160B9]'}`}>{item.icon}</span>
                      <span className="font-medium text-sm">{item.label}</span>
                    </div>
                    {item.count !== undefined && item.count > 0 && (
                      <span className={`text-xs font-bold ${activeSection === item.id ? 'text-[#0160B9]' : 'text-[#0160B9]'}`}>{item.count.toLocaleString()}</span>
                    )}
                  </button>
                ))}
              </nav>
            ...
            {pending.length > 0 && (
              <div className="bg-white rounded-xl overflow-hidden shadow-sm border border-[#e3e2df]">
                <div className="px-4 py-3 border-b border-[#efeeeb] flex items-center justify-between">
                  ...
                  <span className="text-xs font-semibold text-[#0160B9] bg-[#BBDEFB]/40 px-2 py-0.5 rounded-full">{pending.length}</span>
                </div>
                <div className="divide-y divide-[#efeeeb]">
                ...
                {pending.length > 2 && (
                  <button
                    ...
                    className="w-full py-2.5 text-xs font-bold text-[#0160B9] hover:bg-[#faf9f6] transition border-t border-[#efeeeb]"
                  >
