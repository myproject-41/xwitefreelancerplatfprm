  ...
  return (
    <div className="mt-3 rounded-2xl border border-[#e2e8f0] bg-[#f8fafc] p-4">
      ...
      <div className="mb-3 flex items-center gap-3">
        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#E3F2FD] text-sm font-bold text-[#0160B9]">
          {p.freelancerImage
      ...
      {p.freelancerId && (
        <div className="mb-2">
          <Link href={`/profile/${p.freelancerId}`} target="_blank"
            className="inline-block rounded-xl border border-[#90CAF9] bg-[#E3F2FD] px-4 py-2 text-sm font-bold text-[#0160B9]">
            View Profile →
          </Link>
        </div>
      )}
          ...
          <div className="mb-5 lg:mb-0 lg:sticky lg:top-24">
              ...
              {(['all', 'unread'] as const).map(f => (
                <button
                  ...
                  className={`flex-1 rounded-xl px-4 py-2.5 text-sm font-bold transition lg:text-left ${
                    filter === f
                      ? 'bg-[#0160B9] text-white shadow-sm'
                      : 'border border-[#d6dce3] bg-white text-[#6b7280] hover:border-[#0160B9] hover:text-[#0160B9]'
                  }`}
                >
                  {f === 'all'
            ...
            {unreadCount > 0 && (
              <button
                type="button"
                onClick={markAllRead}
                className="mt-3 w-full rounded-xl border border-[#d6dce3] bg-white px-3 py-2 text-xs font-bold text-[#0160B9] transition hover:bg-[#E3F2FD]"
              >
                Mark all as read
              </button>
            )}
